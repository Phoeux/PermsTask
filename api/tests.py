...
from api.models import Plotter, Pattern, PlotterPattern
...
class TestAPI(APITestCase):
    ...
    def test_user_CRUD(self):
        ...
        url = reverse('api:user-detail', kwargs={
            'pk': get_user_model().objects.filter(username=data_4['username']).values_list('pk', flat=True)[0]})

        data_4_upd = {
            "username": "dealers_user_upd"
        }
        response = self.client.patch(url, data_4_upd, format='json')
        ...
        self.client.logout()
        # User creat users
        self.client.login(username='gleb', password='1234')
        url = reverse('api:user-list')
        data_5 = {
            "username": "users_user",
            "password": "1234",
    ...
    def test_plotter_CRUD(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, msg=response.data)
        self.client.logout()
        # Unauthorized user can't creat plotter
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, msg=response.data)
        self.client.logout()
        # Admin creat/update/delete plotter
        self.client.login(username='admin', password='1234')
        data2 = {
            "title": "plotter2",
            "creator": 4,
        ...
        self.client.logout()
        # Dealer can creat/update plotter
        self.client.login(username='dealer', password='1234')
        data3 = {
            "title": "plotter3",
            "creator": 5,
