    ...
    def create(self, request, *args, **kwargs):
        ...
        if self.request.user in dealer_group.user_set.all() or \
            ...
            if self.request.user.id == request.data['creator']:
                ...
                self.perform_create(serializer)
                headers = self.get_success_headers(serializer.data)
                plot_subq = Plotter.objects.get(title=serializer.data['title'])
                if self.request.user in dealer_group.user_set.all():
                ...
                # assign_perm('add_plotter', administrator_group, plot_subq)
                # assign_perm('view_plotter', administrator_group, plot_subq)
                # assign_perm('change_plotter', administrator_group, plot_subq)
                # assign_perm('delete_plotter', administrator_group, plot_subq)
...
class PatternModelView(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if self.request.user in admin_group.user_set.all() and admin_group.permissions.filter(
            ...
            # assign_perm('add_pattern', admin_group, patt_subq)
            # assign_perm('view_pattern', admin_group, patt_subq)
            # assign_perm('change_pattern', admin_group, patt_subq)
            # assign_perm('delete_pattern', admin_group, patt_subq)
